...
export default {
  ...
  mounted () {
    ...
    if (!this.existId !== true) {
      this.getTableData()
    }
  },
  computed: {
    // 把planId缓存到mapstate里
    ...mapState('planId', ['planId', 'existId', 'planType'])
  },
  methods: {
    // 给每一行添加索引
    ...
    saveData () {
      let _this = this
      // console.log(_this.existId)
      let id = !_this.existId === true ? _this.planId : _this.existId
      console.log(id)
      ...
      _this.stingData = JSON.stringify(_this.executiveUnit)
      ...
      _this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error('保存数据失败，请重新选择数据或者新建数据')
        }
      })
    },
    // 展示数据
    getTableData () {
      ...
      // console.log(this.existId)
      _this.$http({
        url: _this.$http.adornUrl('/dive/makePlan/info/' + this.existId),
        method: 'POST'
      }).then(({data}) => {
        console.log(data)
        if (data && data.code === 0) {
          // console.log(data)
          if (!data.data.executiveUnit) {
          } else {
            let jsonData = JSON.parse(data.data.executiveUnit)
            this.divingWorkData = jsonData.divingWorkData
            // 医学保障
            this.medicalSecuryData = jsonData.medicalSecuryData
            // 设备保障
            this.deviceSecuryData = jsonData.deviceSecuryData
          }
        } else {
