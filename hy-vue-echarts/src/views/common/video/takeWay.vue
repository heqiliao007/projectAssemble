  ...
  <div style="margin: 10px 57px 0;">
    <el-row>
      <el-col :span="12">
        <el-breadcrumb separator-class="el-icon-arrow-right" style="border-left: 2px solid rgb(65, 169, 254);padding-left: 6px;"
          ...
          <el-breadcrumb-item>服化道</el-breadcrumb-item>
        </el-breadcrumb>
      </el-col>
      <el-col :span="12">
        <div class="right">
    ...
    <el-row>
      <el-col :span="8" style="padding-right: 56px;">
        <div class="item">
          <span>所属公司：</span>
          <p>{{actorInfo.talentAgency}}</p>
        </div>
        <div class="item">
          <span>代表作品：</span>
          <p>{{actorInfo.listWorks}}</p>
        </div>
        <div class="item unfold" style="min-height: 47px;">
          <span>主要成就：</span>
          <p v-for="(item, index) in actorInfo.achivechList" :key="index">
            {{item.number}}
            <a>{{item.title}}</a>
            {{item.main}}
          </p>
