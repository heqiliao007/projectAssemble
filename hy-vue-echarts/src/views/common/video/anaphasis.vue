    ...
    <el-row>
      <el-col :span="8" style="padding-right: 56px;">
        ...
        <div class="item unfold" style="min-height: 47px;">
          <span>主要成就：</span>
          <p v-for="(item, index) in actorInfo.achievement" :key="index">
            {{item.number}}
            <a>{{item.title}}</a>
            {{item.main}}
          </p>
      ...
      <el-col :span="8" style="padding: 0 28px;">
        <div class="item">
          <span>成立时间：</span>
          <p>{{actorInfo.foundDate}}</p>
        </div>
      </el-col>
      <el-col :span="8" style="padding-Left: 56px;">
        <div class="item">
          <span>标签：</span>
          <p>{{actorInfo.tagNames}}</p>
        </div>
        <!-- <div class="item">
          <span>代表作品：</span>
          <p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getbasis() {
        this.$http({
            url: this.$http.adornUrl('/web/company/getCompanyByCompanyId/' + this.id+'?companyId=' + this.id),
            method: 'POST'
          })
          .then(({
            data
      ...
      getteleplays() {
        this.$http({
            url: this.$http.adornUrl('web/company/listVideoAndStaffByPostProductionCompany?actorId=' + this.id+'&size=6&current=' +
              this.page1),
            method: 'POST'
          })
          .then(({
      ...
      getmovies() {
        this.$http({
            url: this.$http.adornUrl('/web/company/listMovieAndStaffByPostProductionCompany?actorId='+ this.id+'&size=6&current=' +
              this.page2),
            method: 'POST'
          })
          .then(({
