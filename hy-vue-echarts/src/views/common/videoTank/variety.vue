	...
	<div style="overflow: hidden;position: relative;width: 100%;">
		...
		<div class="search" v-if="isSearching == 'true'">
			<div class="search-box">
				<span>日期：</span>
				<el-date-picker size="small" v-model="date" type="date" placeholder="选择日期" style="width: 100px;"></el-date-picker>
				<span>地区：</span>
				...
				<span>类型：</span>
				<el-select size="small" v-model="attributenow" placeholder="请选择" style="width: 130px;">
...
export default {
    ...
    renderHeader(h, { column }) {
      return h('i', {
		...
		getattribute() {
			this.$http({
				url: this.$http.adornUrl('/web/videotype/listAll/3'),
				method: 'POST'
			})
				.then(({ data }) => {
		...
		gettableList() {
			...
			this.$http({
				url: this.$http.adornUrl(
					'/web/videocontent/list?categoryCode=3&size=10&startDate=' +
						curent +
						'&endDate='+curent
						+'&current=' +
						this.page +
