	...
	<div style="overflow: hidden;position: relative;width: 100%;">
			...
			<el-col :span="24">
				<el-card>
					<div slot="header" style="margin:0 14px;" class="clearfix">
						<span style="float: left;font-size:18px">播放量排行</span>
              ...
              <el-table-column :render-header="renderHeader" fixed width="80">
							...
							<el-table-column width="220" prop="formalNumber" label="当前正片播放量（万）">
								<template slot-scope="scope">
									<span>{{ scope.row.formalNumber / 10000 }}</span>
								</template>
							</el-table-column>
		...
		<div class="search" v-if="isSearching == 'true'">
			<div class="search-box">
				...
				<span>地区：</span>
				<el-select size="small" v-model="regionnw" placeholder="请选择" style="width: 150px;"><el-option v-for="item in region" :key="item" :label="item" :value="item"></el-option></el-select>
				<span>上线时间：</span>
				<el-date-picker size="small" v-model="online" type="date" placeholder="开始日期" style="width: 150px;"></el-date-picker>
				<span>属性：</span>
				<el-select size="small" v-model="typenow" placeholder="请选择" style="width: 150px;"><el-option v-for="item in type" :key="item.id" :label="item.name" :value="item.id"></el-option></el-select>
				<span>类型：</span>
...
export default {
    ...
    renderHeader(h, { column }) {
      return h('i', {
		...
		getattribute() {
			this.$http({
				url: this.$http.adornUrl('/web/videotype/listAll/2'),
				method: 'POST'
			})
				.then(({ data }) => {
					console.log(data);
		...
		gettableList() {
			...
			this.$http({
				url: this.$http.adornUrl(
					'/web/videocontent/list?categoryCode=2&size=10&startDate=' +
						curent +
						'&endDate='+curent
						+'&current=' +
						this.page +
