  ...
  <div style="position: relative">
    ...
    <el-row style="background: #ffffff;padding: 0;margin: 20px;">
      <el-col :span="24" style="padding: 0">
        ...
        <div id="J_chartbarBox2" style="height: 430px; width: 96%;" v-loading='chartLoading'></div>
      </el-col>
    </el-row>
    <!--表格-->
    <el-row style="background: #ffffff;padding: 0;margin: 20px;">
      <el-col :span="24" style="padding: 0">
        ...
        <div>
          ...
          >
            ...
            <el-table-column property ="name" min-width='200px;' label="影院名称">
               <template slot-scope="scope">
                  ...
                  cursor:pointer;
                  "
                  @click='router_link(scope.row.cinema_id)'
                  >{{scope.row.name}}</span>
                </template>
            </el-table-column>
          ...
          >
            ...
            <el-table-column :key='index' property ="name" min-width='200px;' label="影院名称">
            </el-table-column>
    ...
    <div class="search" v-if="isSearching=='true'">
      <div class="search-box">
        ...
        <span style="margin-left:56px">影院：</span>
...
export default {
  ...
  mounted() {
    ...
    // this.initChartBar()
			...
			// _this
			// 	.$http({
			// 		url: _this.$http.adornUrl('web/moviecompany/screeningAnalysisWithCinema/14317/2019-04-02'),
			// 		method: 'post',
			// 		data: {}
			// 	})
			// 	.then((res) => {
  ...
  methods: {
    ...
    router_link(id,time){
        sessionStorage.setItem('movieDetails_dataID',id)
         this.$router.push({
            path: '/movieDetails',
            query: {
            	id: id,
    ...
    getData () {
      ...
      _this.alltimes = []
      _this.$http({
        url: _this.$http.adornUrl('/web/moviecompany/screeningAnalysisWithCinema/'+_this.productid+ '/' + _this.time+'/?current='+_this.current+'&size=10'),
        method: 'POST',
        ...
        if (data.code === 0) {
          ...
          _this.total = Number(data.page.total) // 保存最大页
          // 画出图表、
