  ...
  <div style="position: relative">
    ...
    <div class="search" v-if="isSearching=='true'">
      <div class="search-box">
        ...
        <span style="margin-left:56px">影投名称： </span>
        <el-input v-model="companyId" style="width: 150px;">
        </el-input>
...
export default {
  ...
  data() {
    return {
      ...
      headerCellStyle: {"padding-left":18 +'px', color: '#fff',backgroundColor: '#C1C1C1','overflow': 'hidden', 'white-space': 'nowrap', 'text-overflow': 'ellipsis'},
      time:new Date(),
  ...
  methods: {
    ...
    getData () {
      ...
      _this.alltimes = []
      _this.$http({
        url: _this.$http.adornUrl('/web/moviecompany/screeningAnalysisWithInvestment/'+_this.productid+ '/' + _this.time+'/?current='+_this.current+'&size=10'),
        method: 'POST',
        ...
        console.log(data, '获取到影投数据===============')
        if (data.code === 0) {
          var movieData_seat = [ // 排座的表格
            {label: '影投名称', prop: 'name', prop1: ''},
            {label: '总座位数', prop: 'total_arranged_seating', prop1: 'total_arranged_seating_rate'}
          ]
