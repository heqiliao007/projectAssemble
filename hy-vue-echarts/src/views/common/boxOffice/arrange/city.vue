      ...
      <el-col :span="24" style="padding: 0">
        ...
        <div>
          ...
          >
            ...
            <el-table-column prop="value" label="城市名称">
							<template slot-scope="scope">
								<a class="link text_hover" @click="alink(scope.row.id)" style="color:#3295D9;text-decoration:none;cursor: pointer;">{{ scope.row.value }}</a>
							</template>
            </el-table-column>
            <template v-for="(item, index) in movieData_playtimes">
              <el-table-column :key='index' :property ="item.prop" :label="item.label">
                <template slot-scope="scope">
...
export default {
  ...
  data() {
    return {
      ...
      isseat: false,
      tableData: [] // 列表数据 isseat=true的时候，展示排座数，isseat = false的时候展示排片数
    }
  },
  created () {
  ...
  methods: {
    ...
    getData () {
      ...
      // _this.alltimes = []
      _this.$http({
        url: _this.$http.adornUrl('/web/moviecompany/screeningAnalysisWithCity/'+_this.productid+ '/' + _this.time+'/?current='+_this.current+'&size=10'),
        method: 'POST',
        ...
        console.log('获取到新的数据', data)
        if (data.code === 0) {
          var movieData_seat = [ // 排座的表格
            {label: '总排座', prop: 'total_arrangedSeating', prop1: 'total_arranged_seating_rate'}
          ]
